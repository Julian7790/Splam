...
const FavoritesPage = () => {
  const { favorite, setFavorite } = useFavorite(); // Access favorite songs context
  ...
  useEffect(() => {
    const getFavoriteSongs = async () => {
      const songs = await fetchFavoriteSongs();
      setFavorite(songs);
    };
    getFavoriteSongs();
  }, []);
  ...
  const handleFavoriteToggle = async (track) => {
    if (favorite.some((fav) => fav.id === track.id)) {
      // Remove from Firestore
    } else {
      await addFavoriteSong(track);
            ...
            <div key={track.id} className='max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700'>
              ...
              <div className="flex justify-between mt-4">
                <button onClick={() => handleFavoriteToggle(track)}>
                  <img src={redheart} alt='Unfavorite' className='w-12 h-12' />
                </button>

                <button onClick={() => setRepeatMode(!repeatMode)}>
                  <img src={repeatMode ? repeat1 : repeat} alt='Repeat' className='w-12 h-12' />
                </button>
              </div>
            </div>
          ))}
